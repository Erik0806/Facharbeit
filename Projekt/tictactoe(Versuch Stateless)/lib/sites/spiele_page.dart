...
class SpielePage extends StatelessWidget {
  static const String route = '/spiel';

  SpielePage({Key? key, required this.context}) : super(key: key);
  ...
  late ChangeProvider changeProvider = Provider.of<ChangeProvider>(context);
  late List<List<String>> fields = List.generate(
    3,
    (_) => List.generate(3, (_) => ""),
  );
  ...
  Widget build(BuildContext context) {
    return themed(
      ...
      Scaffold(
          ...
          body: layouthelper(context, player)),
    );
  }
  ...
  void setEmptyFields() => fields = List.generate(
        3,
        (_) => List.generate(3, (_) => ""),
      );

  ///Diese Methode wird jedesmal aufgerufen, wenn ein Spielzug gemacht wird
  void onChange(ChangeProvider changeProvider) {
    fields[changeProvider.getX][changeProvider.getY] = player.lastPlayer;
  ...
  Widget playeranzeige() {
    String lPlayer = player.lastPlayer;
    return Center(
      child: Text(
        won
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return IgnorePointer(
            ignoring: ignoring,
            child: Tile(player: player, x: x, y: y),
          );
        },
      ),
    );
